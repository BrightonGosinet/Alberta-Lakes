...
export default function LakeInfo({ lake, currentUser, setPage, onBack }) {
  ...
  const [toast, setToast] = useState(null); // null | "saved" | "already" | "error"
  ...
  const handleAddToSaved = () => {
    saveLake(lake._id)
      .then(() => showToast("Lake saved!"))
      .catch(e => {
        if (e.message === "Lake already saved.") showToast("Already saved!");
    ...
    <div className="page-fade lake-info">
      ...
      {toast && (
        <div style={{
          ...
          background: "#1a3a2a", color: "#fff", padding: "10px 22px",
      ...
      <div className="lake-info-card">
        ...
        <div className="lake-info-card-footer">
          {currentUser ? (
            <button type="button" onClick={handleAddToSaved} className="lake-info-add-icon-btn">
              <img src={addLakeIcon} alt="" className="lake-info-add-icon" />
            </button>
          ) : (
            <button type="button" onClick={() => setPage("login")} className="lake-info-add-icon-btn">
              <img src={addLakeIcon} alt="" className="lake-info-add-icon" />
            </button>
          )}
        </div>
