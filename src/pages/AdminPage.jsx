...
export default function AdminPage() {
  ...
  const approve = id => { setReports(r => r.map(x => x.id===id ? { ...x, status:"approved" } : x)); showToast("Report approved.", "success"); };
  const remove  = id => { setReports(r => r.filter(x => x.id!==id)); showToast("Report deleted.", "danger"); };
